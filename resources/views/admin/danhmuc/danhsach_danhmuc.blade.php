  ...
  <div class="col-xs-12">
    <div class="box">
      <div class="box-header">
        <a href="{{route('danh-muc.create')}}" class="btn btn-success">+Thêm mới Danh Mục</a>
        @if(session('message'))
            ...
            <tr id="{{$ds->id}}">
              ...
              <td class="text-center">
                <div style="display: flex; align-items: center; justify-content: center;">
                    <a style="margin: 5px" href="{{route('danh-muc.edit',$ds->id)}}" class="btn btn-success">Sửa</a>
                    <form action="{{ route('danh-muc.destroy', $ds->id) }}" method="POST">
                        @csrf
                        @method('DELETE')
        ...
        <footer class="panel-footer">
          <div class="row" style="height: 60px;">
            <div class="col-sm-7 text-right ">
              {{ $danhsach_dm->appends(request()->all())->links('pagination::bootstrap-4') }}
            </div>
          </div>
        </footer>
      </div>
